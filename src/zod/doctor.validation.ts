...
export const createDoctorFormZodSchema = z.object({
	...
	address: z
		...
		.refine((value) => value.length === 0 || value.length >= 10, "Address must be at least 10 characters"),
	registrationNumber: z.string().trim().min(1, "Registration number is required"),
	experience: z
		.string()
		.trim()
		.refine((value) => value.length === 0 || /^\d+$/.test(value), "Experience must be an integer"),
	...
	specialties: z.array(z.uuid("Please select a valid specialty")).min(1, "At least one specialty is required"),
});
