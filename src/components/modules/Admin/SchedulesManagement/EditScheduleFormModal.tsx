...
const EditScheduleFormModal = ({ open, onOpenChange, schedule }: EditScheduleFormModalProps) => {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader className="border-b px-6 py-5 pr-14">
					...
					<DialogDescription>Update the date and time range for this schedule slot.</DialogDescription>
				</DialogHeader>
					...
					<div className="px-6 py-5">
						...
						>
							<div className="grid gap-4 sm:grid-cols-2">
								<form.Field name="startDate" validators={{ onChange: editScheduleFormZodSchema.shape.startDate }}>
									{(field) => <AppField field={field} label="Start Date" type="date" />}
								</form.Field>

								<form.Field name="endDate" validators={{ onChange: editScheduleFormZodSchema.shape.endDate }}>
									{(field) => <AppField field={field} label="End Date" type="date" />}
								</form.Field>

								<form.Field name="startTime" validators={{ onChange: editScheduleFormZodSchema.shape.startTime }}>
									{(field) => <AppField field={field} label="Start Time" type="time" />}
								</form.Field>

								<form.Field name="endTime" validators={{ onChange: editScheduleFormZodSchema.shape.endTime }}>
