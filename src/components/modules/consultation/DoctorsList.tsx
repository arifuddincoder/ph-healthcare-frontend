...
const DoctorsList = ({
	...
	return (
		<section className="space-y-6 pb-8">
			<div className="relative overflow-hidden rounded-2xl border bg-linear-to-br from-cyan-50 via-white to-blue-50 p-6">
				...
				<div className="relative space-y-3">
					...
					<p className="max-w-3xl text-sm text-muted-foreground sm:text-base">
						Discover trusted doctors, compare experience and fees, and open detailed profiles to find the right
						specialist.
					</p>
				</div>
			</div>
			...
			{isBusy && <div className="rounded-md border p-4 text-sm text-muted-foreground">Loading doctors...</div>}
			...
			{!isBusy && doctors.length > 0 && (
				<>
								...
								>
									...
									<div className="mt-auto grid gap-2 pt-5 sm:grid-cols-2">
										...
										<Button asChild className="w-full">
											<Link href={`/consultation/doctor/${doctor.id}`}>View Details</Link>
										</Button>
									</div>
								</article>
							);
					...
					<div className="space-y-3 pt-2">
						...
						<p className="text-center text-sm text-muted-foreground">Total {meta?.total ?? doctors.length} doctors</p>
					</div>
				</>
			)}
		</section>
