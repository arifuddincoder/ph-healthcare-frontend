...
import { createSchedule, deleteSchedule, getScheduleById, updateSchedule } from "@/services/schedule.services";
import { type ApiErrorResponse, type ApiResponse } from "@/types/api.types";
import { type ICreateSchedulePayload, type ISchedule, type IUpdateSchedulePayload } from "@/types/schedule.types";
...
export const deleteScheduleAction = async (id: string): Promise<ApiResponse<boolean> | ApiErrorResponse> => {
	if (!id) {
		return {
			success: false,
			message: "Invalid schedule id",
...
export const getScheduleByIdAction = async (id: string): Promise<ApiResponse<ISchedule> | ApiErrorResponse> => {
	if (!id) {
		return {
			success: false,
			message: "Invalid schedule id",
