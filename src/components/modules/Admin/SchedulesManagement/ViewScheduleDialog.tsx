...
const ViewScheduleDialog = ({ open, onOpenChange, schedule }: ViewScheduleDialogProps) => {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-h-[90vh] w-[calc(100vw-1.5rem)] max-w-4xl gap-0 overflow-hidden p-0">
				<DialogHeader className="border-b px-6 py-5 pr-14">
					...
					<DialogDescription>Review schedule timing, doctor assignments, and linked appointments.</DialogDescription>
				</DialogHeader>

				<ScrollArea className="max-h-[calc(90vh-5.5rem)]">
					<div className="space-y-4 px-6 py-5">
						{(isLoading || isFetching) && (
							<div className="rounded-md border p-4 text-sm text-muted-foreground">Loading schedule details...</div>
						)}
								...
								<div className="grid gap-4 md:grid-cols-2">
									<div className="rounded-lg border p-4">
										...
										<div className="space-y-2 text-sm">
											<p>
												<span className="font-medium">Schedule ID:</span> {scheduleDetails.id}
											</p>
											<p>
												<span className="font-medium">Start:</span> {formatDateTime(scheduleDetails.startDateTime)}
											</p>
											<p>
												<span className="font-medium">End:</span> {formatDateTime(scheduleDetails.endDateTime)}
											</p>
											<p>
												<span className="font-medium">Duration:</span>{" "}
												{getDurationLabel(scheduleDetails.startDateTime, scheduleDetails.endDateTime)}
											</p>
											<p>
												<span className="font-medium">Created:</span> {formatDateTime(scheduleDetails.createdAt)}
											</p>
											<p>
												<span className="font-medium">Updated:</span> {formatDateTime(scheduleDetails.updatedAt)}
											</p>
										</div>
									</div>
									...
									<div className="rounded-lg border p-4">
										...
										<div className="flex flex-wrap gap-2">
											<Badge variant="secondary">{scheduleDetails.doctorSchedules?.length ?? 0} doctor slots</Badge>
											<Badge variant="secondary">{bookedCount} booked</Badge>
											<Badge variant="secondary">{scheduleDetails.appointments?.length ?? 0} appointments</Badge>
										</div>
									</div>
								</div>
										...
										<div className="space-y-2">
											{(scheduleDetails.doctorSchedules ?? []).slice(0, 10).map((item, index) => (
												<div key={`${item.doctorId}-${index}`} className="rounded-md border p-2 text-sm">
													<p>
														<span className="font-medium">Doctor ID:</span> {item.doctorId}
													</p>
													<p>
										...
										<div className="space-y-2">
											{(scheduleDetails.appointments ?? []).slice(0, 10).map((item, index) => (
												<div key={item.id ?? `appointment-${index}`} className="rounded-md border p-2 text-sm">
													<p>
														<span className="font-medium">Appointment ID:</span> {item.id ?? "N/A"}
													</p>
													<p>
													...
													<p>
														...
														{item.patient?.name ?? item.patient?.email ?? "N/A"}
													</p>
												</div>
											))}
