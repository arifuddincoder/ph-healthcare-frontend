...
export const getMyDoctorSchedules = async (queryString: string) => {
	try {
		return await httpClient.get<IDoctorSchedule[]>(
			queryString ? `/doctor-schedules/my-doctor-schedules?${queryString}` : "/doctor-schedules/my-doctor-schedules",
		);
	} catch (error) {
...
export const updateMyDoctorSchedule = async (payload: IUpdateDoctorSchedulePayload) => {
	try {
		return await httpClient.patch<{ count: number }>("/doctor-schedules/update-my-doctor-schedule", payload);
	} catch (error) {
