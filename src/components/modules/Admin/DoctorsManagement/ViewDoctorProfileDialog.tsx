...
const ViewDoctorProfileDialog = ({ open, onOpenChange, doctor }: ViewDoctorProfileDialogProps) => {
				...
				<ScrollArea className="max-h-[calc(90vh-5.5rem)]">
					<div className="space-y-4 px-6 py-5">
						{(isLoading || isFetching) && (
							<div className="rounded-md border p-4 text-sm text-muted-foreground">Loading doctor details...</div>
						)}
								...
								<div className="grid gap-4 md:grid-cols-2">
									<div className="rounded-lg border p-4">
										...
										<div className="space-y-2 text-sm">
											<p>
												<span className="font-medium">Name:</span> {doctorDetails.name || "N/A"}
											</p>
											<p>
												<span className="font-medium">Email:</span> {doctorDetails.email || "N/A"}
											</p>
											<p>
												<span className="font-medium">Contact:</span> {doctorDetails.contactNumber || "N/A"}
											</p>
											<p>
												<span className="font-medium">Gender:</span> {doctorDetails.gender || "N/A"}
											</p>
											<p>
												<span className="font-medium">Registration:</span> {doctorDetails.registrationNumber || "N/A"}
											</p>
											<p>
												<span className="font-medium">Experience:</span> {doctorDetails.experience ?? 0} years
											</p>
											<p>
												<span className="font-medium">Fee:</span> {doctorDetails.appointmentFee ?? "N/A"}
											</p>
											<p>
												<span className="font-medium">Qualification:</span> {doctorDetails.qualification || "N/A"}
											</p>
											<p>
												<span className="font-medium">Designation:</span> {doctorDetails.designation || "N/A"}
											</p>
											<p>
											...
											<p>
												<span className="font-medium">Address:</span> {doctorDetails.address || "N/A"}
											</p>
										</div>
									</div>
									...
									<div className="rounded-lg border p-4">
										...
										<div className="space-y-2 text-sm">
											<p>
												<span className="font-medium">User Name:</span> {doctorDetails.user?.name || "N/A"}
											</p>
											<p>
												<span className="font-medium">User Email:</span> {doctorDetails.user?.email || "N/A"}
											</p>
											<p>
												<span className="font-medium">Role:</span> {doctorDetails.user?.role || "N/A"}
											</p>
											<p>
												<span className="font-medium">Status:</span> {doctorDetails.user?.status || "N/A"}
											</p>
											<p>
											...
											<p>
												<span className="font-medium">Created:</span> {formatDateTime(doctorDetails.user?.createdAt)}
											</p>
											<p>
												<span className="font-medium">Updated:</span> {formatDateTime(doctorDetails.user?.updatedAt)}
											</p>
										</div>
									</div>
								</div>
										...
										<div className="space-y-2">
											{(doctorDetails.reviews ?? []).slice(0, 5).map((review, index) => (
												<div key={review.id ?? `review-${index}`} className="rounded-md border p-2 text-sm">
													<p>
														<span className="font-medium">Rating:</span> {review.rating ?? "N/A"}
													</p>
													<p>
														<span className="font-medium">Comment:</span> {review.comment || "N/A"}
													</p>
									...
									<div className="rounded-lg border p-4">
										...
										<div className="space-y-2">
											{(doctorDetails.doctorSchedules ?? []).slice(0, 8).map((item, index) => (
												...
												>
													<p>
														<span className="font-medium">Start:</span> {formatDateTime(item.schedule?.startDateTime)}
													</p>
													<p>
														<span className="font-medium">End:</span> {formatDateTime(item.schedule?.endDateTime)}
													</p>
													<p>
														<span className="font-medium">Booked:</span> {item.isBooked ? "Yes" : "No"}
													</p>
												</div>
											))}
									...
									<div className="space-y-2">
										{(doctorDetails.appointments ?? []).slice(0, 10).map((appointment, index) => (
											<div key={appointment.id ?? `appointment-${index}`} className="rounded-md border p-2 text-sm">
												<p>
													<span className="font-medium">Patient:</span> {appointment.patient?.name || "N/A"}
												</p>
												<p>
													<span className="font-medium">Patient Email:</span> {appointment.patient?.email || "N/A"}
												</p>
												<p>
													<span className="font-medium">Status:</span> {appointment.status || "N/A"}
												</p>
												<p>
												...
												<p>
													<span className="font-medium">Has Prescription:</span>{" "}
