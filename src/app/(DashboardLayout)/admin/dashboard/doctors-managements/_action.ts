...
export const deleteDoctorAction = async (id: string): Promise<ApiResponse<{ message: string }> | ApiErrorResponse> => {
	if (!id) {
		return {
			success: false,
...
export const getDoctorByIdAction = async (id: string): Promise<ApiResponse<IDoctorDetails> | ApiErrorResponse> => {
	if (!id) {
		return {
			success: false,
			message: "Invalid doctor id",
