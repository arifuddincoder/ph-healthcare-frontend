...
export const doctorColumns: ColumnDef<IDoctor>[] = [
	//id or accessorKey is same as the key in the data object
	{
		...
		header: "Doctor",
		cell: ({ row }) => (
			<UserInfoCell name={row.original.name} email={row.original.email} profilePhoto={row.original.profilePhoto} />
		),
	},
	{
		id: "specialties",
	...
	},
	{
		...
		header: "Experience",
		cell: ({ row }) => {
			return <span className="text-sm font-medium">{row.original.experience ?? 0} years</span>;
		},
	},
	{
		id: "appointmentFee",
		...
		cell: ({ row }) => {
			return (
				<div className="flex items-center gap-1">
					...
					<span className="text-sm font-medium">{row.original.averageRating?.toFixed(1) || "0.0"}</span>
				</div>
			);
		},
	},
	{
		...
		header: "Gender",
		cell: ({ row }) => {
			return <span className="text-sm capitalize">{row.original.gender.toLowerCase()}</span>;
		},
	},
	{
		id: "status",
