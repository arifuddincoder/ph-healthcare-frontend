...
import { editDoctorFormZodSchema, type IEditDoctorFormValues } from "@/zod/doctor.validation";
...
const EditDoctorFormModal = ({
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const originalSpecialtyIds = new Set(doctor.specialties.map((item) => item.specialty.id));
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				<DialogHeader className="border-b px-6 py-5 pr-14">
					...
					<DialogDescription>Update doctor profile information and specialties.</DialogDescription>
				</DialogHeader>
					...
					<div className="px-6 py-5">
						...
						>
							<div className="grid gap-4 md:grid-cols-2">
								<form.Field name="name" validators={{ onChange: editDoctorFormZodSchema.shape.name }}>
								...
								<form.Field name="contactNumber" validators={{ onChange: editDoctorFormZodSchema.shape.contactNumber }}>
								...
								<form.Field name="experience" validators={{ onChange: editDoctorFormZodSchema.shape.experience }}>
									{(field) => (
								...
								<form.Field name="qualification" validators={{ onChange: editDoctorFormZodSchema.shape.qualification }}>
								...
								<form.Field name="designation" validators={{ onChange: editDoctorFormZodSchema.shape.designation }}>
									{(field) => <AppField field={field} label="Designation" placeholder="Enter designation" />}
								</form.Field>

								<form.Field name="gender" validators={{ onChange: editDoctorFormZodSchema.shape.gender }}>
									{(field) => {
										const firstError =
										...
										return (
											<div className="space-y-1.5">
												<Label htmlFor="edit-doctor-gender" className={cn(firstError && "text-destructive")}>
													Gender
												</Label>
												<Select
								...
								<form.Field name="address" validators={{ onChange: editDoctorFormZodSchema.shape.address }}>
									{(field) => {
										const firstError =
											field.state.meta.isTouched && field.state.meta.errors.length > 0
												? field.state.meta.errors[0]
							...
							<form.Field name="specialties" validators={{ onChange: editDoctorFormZodSchema.shape.specialties }}>
								{(field) => {
									const firstError =
										field.state.meta.isTouched && field.state.meta.errors.length > 0
											? field.state.meta.errors[0]
