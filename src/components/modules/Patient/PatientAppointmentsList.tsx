			...
			{sortedAppointments.length === 0 ? (
				<Card>
					<CardHeader>
						...
						<CardDescription>Start by choosing a doctor and selecting an available time slot.</CardDescription>
					</CardHeader>
					<CardFooter>
						<Button asChild>
				...
				<div className="grid gap-4 lg:grid-cols-2">
					{sortedAppointments.map((appointment) => {
						const canPayNow = appointment.paymentStatus !== "PAID" && appointment.status !== "CANCELED";
								...
								<CardHeader>
									<div className="flex flex-wrap items-start justify-between gap-3">
										<div className="space-y-1">
											...
											<CardDescription>{appointment.doctor?.designation || "Consultation"}</CardDescription>
										</div>
									...
									<div className="grid gap-3 sm:grid-cols-2">
										...
										<div className="rounded-2xl border bg-muted/20 p-4 text-sm">
											...
											<p className="text-muted-foreground">
												Transaction {appointment.payment?.transactionId || "Pending"}
											</p>
										</div>
									</div>
